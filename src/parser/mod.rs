pub use cooklang::{CooklangParser, Converter, Extensions, ScalableRecipe};
...
Add @eggs{2} to the bowl."#;
        ...
        let recipe = result.unwrap();
        assert_eq!(recipe.metadata.map.get("author"), Some(&"John Doe".to_string()));
        assert_eq!(recipe.metadata.map.get("tags"), Some(&"breakfast, quick".to_string()));
    }
